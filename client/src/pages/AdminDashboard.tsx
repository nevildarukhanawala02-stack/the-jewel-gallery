...
function AdminLayout({ children, title }: { children: React.ReactNode; title: string }) {
    ...
    <>
      ...
      <div style={{
        ...
        <div>
          ...
          <div style={{ fontSize: "9px", fontWeight: 600, letterSpacing: "2px", textTransform: "uppercase", color: "var(--text-muted)", marginTop: "4px" }}>
            Admin Console
          </div>
        </div>
      ...
      {/* Bottom */}
      <div style={{ padding: "16px 24px", borderTop: "1px solid var(--linen-dark)" }}>
        <button
  ...
  return (
    <div style={{ display: "flex", minHeight: "100vh", background: "#F8F5F0" }}>
      ...
      >
        <div style={{ padding: "20px 24px 16px", borderBottom: "1px solid var(--linen-dark)", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          <div>
            <div style={{ fontFamily: "var(--font-display)", fontSize: "16px", fontWeight: 400, color: "var(--text-dark)", letterSpacing: "1px" }}>
              The Jewel <span style={{ color: "var(--gold)", fontStyle: "italic" }}>Gallery</span>
            </div>
            <div style={{ fontSize: "9px", fontWeight: 600, letterSpacing: "2px", textTransform: "uppercase", color: "var(--text-muted)", marginTop: "4px" }}>
              Admin Console
        ...
        <div style={{ padding: "16px 24px", borderTop: "1px solid var(--linen-dark)" }}>
          <button onClick={() => { navigate("/"); setSidebarOpen(false); }} style={{ display: "block", width: "100%", padding: "10px 0", background: "none", border: "none", color: "var(--text-muted)", fontSize: "12px", cursor: "pointer", textAlign: "left", marginBottom: "8px" }}>
            ← View Storefront
          </button>
          <button onClick={logout} style={{ display: "block", width: "100%", padding: "10px 0", background: "none", border: "none", color: "var(--text-muted)", fontSize: "12px", cursor: "pointer", textAlign: "left" }}>
            Sign Out
          </button>
        </div>
      </aside>
      ...
      <main style={{ flex: 1, overflow: "auto", minWidth: 0 }}>
        ...
        <div style={{
          ...
          <div style={{ display: "flex", alignItems: "center", gap: "16px" }}>
            ...
            <h1 style={{ fontFamily: "var(--font-display)", fontSize: "20px", fontWeight: 300, color: "var(--text-dark)", margin: 0 }}>
              {title}
            </h1>
          </div>
          <div className="admin-topbar-date" style={{ fontSize: "11px", color: "var(--text-muted)" }}>
  ...
  if (!user) {
    return (
      <div style={{ display: "flex", flexDirection: "column", justifyContent: "center", alignItems: "center", minHeight: "100vh", background: "#F8F5F0", gap: "24px" }}>
        ...
        <p style={{ color: "var(--text-muted)", fontSize: "14px", margin: 0, textAlign: "center", maxWidth: "320px" }}>Your session has expired. Please sign in again to access the admin dashboard.</p>
        <a href={getLoginUrl("/admin")} style={{ background: "var(--gold)", color: "#fff", padding: "12px 32px", borderRadius: "4px", textDecoration: "none", fontWeight: 600, fontSize: "13px", letterSpacing: "0.1em" }}>SIGN IN</a>
        <a href="/" style={{ color: "var(--text-muted)", fontSize: "13px", textDecoration: "none" }}>Go to Homepage</a>
      </div>
    );
  }
  ...
  return (
    <AdminLayout title="Operations Dashboard">
          ...
          return (
            <div key={card.label} style={{
              ...
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: "10px" }}>
                <div style={{ fontSize: "9px", fontWeight: 600, letterSpacing: "1.5px", textTransform: "uppercase", color: "var(--text-muted)" }}>
                  {card.label}
                </div>
                <Icon size={14} color={card.color} />
              </div>
              <div style={{ fontFamily: "var(--font-display)", fontSize: "24px", fontWeight: 300, color: "var(--text-dark)", marginBottom: "4px" }}>
                {card.value}
              </div>
      ...
      <div className="admin-dashboard-grid" style={{ display: "grid", gridTemplateColumns: "1fr 300px", gap: "20px", marginBottom: "28px" }}>
        {/* Recent Orders */}
        <div style={{ background: "#fff", border: "1px solid var(--linen-dark)", overflow: "hidden", boxShadow: "0 1px 4px rgba(0,0,0,0.04)" }}>
          <div style={{
            padding: "14px 18px",
            borderBottom: "1px solid var(--linen-dark)",
            display: "flex",
          ...
          <div style={{ overflowX: "auto" }}>
            <table style={{ width: "100%", borderCollapse: "collapse", minWidth: "480px" }}>
              <thead>
                <tr style={{ borderBottom: "1px solid var(--linen-dark)", background: "var(--linen)" }}>
              ...
              <tbody>
                {recentOrders && recentOrders.length > 0 ? recentOrders.map((order) => (
                  <tr key={order.id} style={{ borderBottom: "1px solid var(--linen-dark)" }}>
                    <td style={{ padding: "10px 14px", fontSize: "12px", color: "var(--gold)", fontFamily: "var(--font-body)", whiteSpace: "nowrap" }}>
                      {order.orderNumber}
                    </td>
                    <td style={{ padding: "10px 14px", fontSize: "12px", color: "var(--text-dark)", whiteSpace: "nowrap" }}>
                  ...
                  <tr>
                    <td colSpan={5} style={{ padding: "40px", textAlign: "center", color: "var(--text-muted)", fontSize: "12px" }}>
                      No orders yet
                    </td>
                  </tr>
                )}
        ...
        {/* Low Stock Alerts */}
        <div style={{ background: "#fff", border: "1px solid var(--linen-dark)", height: "fit-content", boxShadow: "0 1px 4px rgba(0,0,0,0.04)" }}>
          <div style={{
            padding: "14px 18px",
            borderBottom: "1px solid var(--linen-dark)",
            display: "flex",
            ...
            {lowStockProducts && lowStockProducts.length > 0 ? lowStockProducts.map((p) => (
              <div key={p.id} style={{
                ...
                <div style={{ minWidth: 0 }}>
                  <div style={{ fontSize: "12px", color: "var(--text-dark)", marginBottom: "2px", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{p.name}</div>
              ...
              <div style={{ padding: "32px 18px", textAlign: "center", color: "var(--text-muted)", fontSize: "12px" }}>
      ...
      <div style={{
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "10px", marginBottom: "20px" }}>
          ...
          <h3 style={{ fontFamily: "var(--font-display)", fontSize: "15px", fontWeight: 400, color: "var(--text-dark)", letterSpacing: "1px", margin: 0 }}>
            Homepage Hero Image
          </h3>
        </div>
        <div className="admin-hero-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "20px", alignItems: "start" }}>
          ...
          <div>
            <div style={{ fontSize: "11px", color: "var(--text-muted)", letterSpacing: "1px", textTransform: "uppercase", marginBottom: "10px" }}>Upload New Image</div>
            <input
              ref={heroFileRef}
              type="file"
              accept="image/*"
      ...
      <div style={{
        ...
        {migrateResult && (
          <div style={{ marginTop: "20px", padding: "16px", background: "var(--linen)", borderRadius: "4px", border: "1px solid var(--linen-dark)" }}>
            <div style={{ display: "flex", gap: "24px", marginBottom: migrateResult.errors.length > 0 ? "16px" : 0 }}>
              <div style={{ textAlign: "center" }}>
                <div style={{ fontSize: "22px", fontWeight: 600, color: "var(--text-dark)" }}>{migrateResult.total}</div>
                <div style={{ fontSize: "10px", color: "var(--text-muted)", letterSpacing: "1px", textTransform: "uppercase" }}>Total</div>
              </div>
              <div style={{ textAlign: "center" }}>
                <div style={{ fontSize: "22px", fontWeight: 600, color: "#10B981" }}>{migrateResult.migrated}</div>
                <div style={{ fontSize: "10px", color: "var(--text-muted)", letterSpacing: "1px", textTransform: "uppercase" }}>Migrated</div>
              </div>
              <div style={{ textAlign: "center" }}>
                <div style={{ fontSize: "22px", fontWeight: 600, color: migrateResult.failed > 0 ? "#EF4444" : "var(--text-muted)" }}>{migrateResult.failed}</div>
                <div style={{ fontSize: "10px", color: "var(--text-muted)", letterSpacing: "1px", textTransform: "uppercase" }}>Failed</div>
              </div>
            </div>
            {migrateResult.errors.length > 0 && (
              <div style={{ fontSize: "11px", color: "#EF4444", lineHeight: 1.8 }}>
